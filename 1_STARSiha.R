...
sites_path <- paste0(Myfiles,"Kinzig_IHApoints.gpkg")
...
import_data(dem = dem_path, sites = sites_path, streams = streams_path,
            predictor_raster = c(Elevation), predictor_r_names = c("Elevation"),
...
tmap_mode("plot")
tm_shape(Elevation) +
  tm_raster("Elevation", palette = terrain.colors(n = 9), n = 9,
...
calc_attributes_edges(input_raster ="dem",
                      stat_rast = "mean",
                      attr_name_rast= "dem", round_dig = 2)
calc_attributes_sites_approx(sites_map = "sites",
                             input_attr_name = "dem",
                             output_attr_name = "dem",
                             stat = "mean", overwrite = TRUE)
...
execGRASS("g.copy",
          parameters = list(
            vector = "sites,sites_t"
          ))

ssn_dir_kinzig <- file.path(Myfiles, "KinzigIHA.ssn")
export_ssn(ssn_dir_kinzig, delete_directory = TRUE)
